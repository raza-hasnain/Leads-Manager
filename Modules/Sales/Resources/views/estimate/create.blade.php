{!!Form::open(['route'=>'estimates.add','id'=>'create-estimate-form','enctype' => 'multipart/form-data']) !!}
        ...
        <div class="card mb-4">
            <div class="card-header">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        <h5 class="mb-0">@lang('sales.estimate_details')</h5>
                    </div>
                </div>
            </div>
            <div class="card-body">
                    ...
                    <div class="col-lg-2">
                        ...
                        <select name="status_id" class="form-control">
                            @foreach($statuses as $status) @required
                                <option  value="{{ clean($status->id) }}" @if($status->id == 1)selected @endif>{{ clean($status->name) }}</option>
                            @endforeach
                        </select>
                    </div>
...
{!! Form::close() !!}
<script src="{{asset('public/admin_layout/js/Modules/Estimates/create_estimate.js')}}?v={{ $version }}"></script>
